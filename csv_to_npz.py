...
if __name__ == '__main__':
    ...
    config_saver_path = root_dir.join('custom', 'config_saver_settings.json')
    ...
    for npz_data_folder in npz_data_folders:
        try:
            ...
            raise FileNotFoundError(
                f'No config.json in {npz_data_folder}. Please delete the folder.'
            ) from err
    ...
    for idx, csv_data_path in enumerate(csv_data_lst, start=1):
        ...
        print(
            f'\rFinished: {idx / num_csv_data * 100:>6.2f}% '
            f'[{idx}/{num_csv_data}]',
            end='')
